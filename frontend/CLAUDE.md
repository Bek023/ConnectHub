...
ConnectHub frontend: an Angular 22 standalone-component web app consuming the NestJS backend at `../backend` (contract documented in `backend/API_DOCS.md`). This is a from-scratch rewrite replacing an earlier Flutter frontend — there is no Figma/design doc to follow; UI is built directly against the backend's REST/WebSocket contract. The app is built in phases mirroring the backend's own build order; `TODO.md` in this folder is the live checklist. Phases 1-5 and 7 are done (auth + shell, goals/groups/channels, feed/posts, chat, calls, profile/settings). Phase 6 (notifications, search, web push) and Phase 8 (CI/CD) are open, and each finished phase still carries a short "Qolgan" list of deferred items — read `TODO.md` before assuming a feature is complete.
...
**Routing** (`src/app/app.routes.ts`): flat list of lazy `loadComponent` routes for the public auth pages, plus one `authGuard`-protected parent route (`path: ''`) that lazy-loads `AppShell` (`src/app/shared/layouts/app-shell/app-shell.ts` — sidebar/topbar chrome) and nests every real feature (`feed`, `goals`, `groups`, `channels`, `chat`, `calls`, `notifications`, `profile`, `settings`) as children. Every one of those is a real feature now except `notifications`, which is still the reusable `ComingSoon` placeholder (`src/app/shared/components/coming-soon/coming-soon.ts`) — when building it out, replace that route's `loadComponent` target rather than restructuring the route tree. `calls` has a child route (`calls/:id`, the call room) and is the only feature that takes a query param (`?type=audio|video`).
...
**Chat attachments cover all four media types.** `MEDIA_RULES` in `core/services/media/media.service.ts` mirrors the backend's `ALLOWED_MIME_TYPES` and `MAX_SIZES`; `detectMediaType(file.type)` picks the type, which doubles as the message's `messageType`, and `MessageBubble` switches on it (`<video controls>`, `<audio controls>`, or a download link). Keep the two tables in sync — the backend re-validates and also sniffs magic bytes, so a mime the frontend allows but the backend does not produces a 400 the user cannot act on.

**File attachments are readable only because the dev bucket is world-readable.** `MediaService.putObject` passes `publicRead = mediaType !== 'file'`, so file objects get no `public-read` ACL, and `getPresignedUrl` calls `assertOwner` — it only signs for the **uploader**, keyed on `key.split('/')[1] === userId`. A recipient in the chat can therefore not obtain a presigned URL for someone else's attachment. It works locally only because the bucket-wide `mc anonymous set download` policy overrides the per-object ACL. Against a properly locked bucket, chat file attachments would 403 for everyone but the sender; fixing it means authorizing presigned URLs by chat membership rather than key ownership.

**Media/avatar upload** goes through `MediaService.upload(file, 'image')` → `POST /media/upload` (multipart, `file` + `type`). The backend converts images to `.webp`, generates a thumbnail, stores both in S3/MinIO and returns `{url, key}`; that `url` is then saved onto the user via `PUT /users/me {avatarUrl}`. **This needs MinIO running** (`S3_ENDPOINT=http://localhost:9000`). It is now started via Homebrew, not Docker — see the root `CLAUDE.md` for the exact commands and the required `mc anonymous set download` policy. Without it, upload fails at request time; without the public-download policy the upload succeeds but every `<img>` gets a 403, since media is loaded by direct URL rather than presigned.
...
`CallSocketService` (`core/services/socket/call-socket.service.ts`) owns the `/calls` connection and the whole mediasoup handshake. Three things are load-bearing:

1. **The gateway answers in two different shapes, and you cannot pick one.** Nest emits a handler's return value as a *separate event* when it looks like a `WsResponse` (`{event, data}`) and as a *socket.io ack* otherwise. In `CallGateway` that split is: `joinCallRoom` → `joinedCallRoom` event, `connectTransport` → `transportConnected` event, `resumeConsumer` → `consumerResumed` event; while `createTransport`, `produce`, `consume` and `getProducers` come back as acks. The private `request(event, payload, responseEvent?)` helper therefore registers the event listener **and** passes an ack callback, resolving on whichever arrives first (10s timeout). If you add a gateway handler, check which shape it returns before wiring the client call.
2. **Signaling order is fixed:** `joinCallRoom` → `Device.load(routerRtpCapabilities)` → `createTransport` send + recv → `produce` each local track → `consume` every producer from `joinedCallRoom.producers` and from later `newProducer` events → `resumeConsumer`. The server creates video consumers **paused** (`paused: producer.kind === 'video'`), so skipping `resumeConsumer` yields a black tile with no error anywhere.
3. **Mic/camera toggles pause the producer, they do not stop the track.** `producer.pause()`/`.resume()` keeps the transport and the peer's consumer alive; replacing or stopping tracks would force every remote peer to re-consume.

Remote media is kept as one `MediaStream` per `userId` (audio and video tracks are added to the same stream as they arrive), which is why `RemotePeer.hasVideo` is derived from `stream.getVideoTracks().length` rather than from the producer `kind`.

**Incoming calls arrive on `/notifications`, not `/calls`** — `NotificationSocketService` listens for `incomingCall`/`callEnded`, and `IncomingCallOverlay` is mounted once in `AppShell` so the prompt shows on any page. A user only enters the `/calls` namespace after accepting (`POST /calls/:id/join` → navigate to `/calls/:id`). Leaving the `CallRoom` route tears the socket down in `ngOnDestroy`, so an in-progress call does **not** survive navigation yet.
...
**Own messages come back twice, `newMessage` first.** `ChatGateway.handleSendMessage` broadcasts `newMessage` to the whole room (the sender included) and *then* returns a `WsResponse`, which Nest emits as `messageSent` to the sender only — measured order is `newMessage` → `messageSent`. Optimistic sending therefore cannot simply "append on newMessage, replace on messageSent"; that duplicates every message you send. `ChatRoom.reconcile()` is the single entry point for both events: if the id is already rendered it just retires the matching pending entry, otherwise it swaps the optimistic bubble for the saved one, otherwise it appends. Pending sends are matched by `content` + `mediaUrl` because `SendMessageDto` validates with `forbidNonWhitelisted: true` — you **cannot** thread a client-generated correlation id through the payload without changing the DTO. A send that gets neither event within 10s, or triggers an `exception`, is marked `failed` and offers a retry.
