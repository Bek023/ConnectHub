...
## Infratuzilma / umumiy
- [x] **Real Docker Compose stack to'liq sinaldi va ishladi** — bu sandbox'da endi Docker mavjud ekan (avval yo'q edi). `postgres`+`redis`+`api`+`nginx`ni haqiqiy ishga tushirib, **ilova birinchi marta tarixida** haqiqiy Postgres'ga ulanib, migratsiyalarni ishlatib, 8 ta mediasoup worker bilan muvaffaqiyatli boot bo'ldi; `nginx` orqali HTTPS'da `/api/v1/health` 200 qaytardi, HTTP→HTTPS redirect ishladi, winston fayl logging (`logs/combined.log`) konteyner ichida ham tasdiqlandi. Yo'lda **yana ikkita jiddiy production bug** topildi va **tuzatildi** (pastda). `elasticsearch`/`minio` bilan to'liq stack hali sinalmadi
- [x] **MUHIM (production bug, haqiqiy `docker build` orqali topildi):** `Dockerfile` `node:20-alpine` ishlatardi (musl libc). `mediasoup@3.19.3` bu kombinatsiya uchun prebuilt binary topa olmaydi va manbadan qurishga urinadi, lekin Alpine'da Python yo'q — **`docker build` butunlay muvaffaqiyatsiz tugardi**. Ikkala stage (`builder`, `production`) `node:20-bookworm-slim`ga (Debian/glibc) o'tkazildi, `ffmpeg` o'rnatish `apk add` → `apt-get install`ga o'zgartirildi. Qayta qurib, `docker compose build api` muvaffaqiyatli tasdiqlandi
- [x] **MUHIM (production bug, haqiqiy `docker build` orqali topildi):** `.dockerignore` fayli umuman yo'q edi — `COPY . .` host mashinadagi mahalliy `node_modules`ni (masalan macOS native binary'lari bilan) konteynerning ichidagi `npm install` natijasida yaratilgan **to'g'ri Linux binary'larini ustidan yozib qo'yardi**, natijada `argon2.node: invalid ELF header` xatosi bilan boot bo'lmasdi. `.dockerignore` qo'shildi (`node_modules/`, `.git/`, `dist/` va h.k.)
- [x] **MUHIM (production bug, haqiqiy `docker run` orqali topildi):** `main.ts`da `import * as morgan from 'morgan'` — xuddi avvalgi `supertest` xatosi kabi, `morgan`ning butun `module.exports`i bitta chaqiriladigan funksiya, `esModuleInterop`dagi `import * as` esa uni chaqirib bo'lmaydigan oddiy obyektga aylantiradi. Natijada **`node dist/main.js` (productiondagi haqiqiy CMD) bootstrap paytida `TypeError: morgan is not a function` bilan har doim yiqilardi** — `tsc` buni sezmadi (faqat `supertest`ni sezgan edi), chunki `@types/morgan` boshqacha modellashtirilgan. `import morgan from 'morgan'`ga to'g'rilandi. Qolgan barcha `import * as X` namespace import'lar tekshirildi — faqat shu ikkisi (`morgan`, `supertest`) xavfli ekan, qolganlari (`argon2`, `mediasoup`, `winston`, `path`, `fs`, `os`, `dotenv`) faqat nomli xususiyatlar orqali ishlatiladi, xavfsiz
- [x] `nginx/nginx.conf` — to'liq SSL/TLS qo'shildi: 80-port 443'ga redirect qiladi, 443 `ssl_certificate`/`ssl_certificate_key` (`/etc/nginx/ssl/fullchain.pem`/`privkey.pem`, `docker-compose.yml`da allaqachon mount qilingan), TLS 1.2/1.3, HSTS. `ssl/generate-self-signed.sh` qo'shildi — local/dev uchun soxta sertifikat yaratadi (production uchun haqiqiy sertifikat `ssl/`ga qo'lda qo'yiladi). **Bonus xato tuzatildi:** upstream `api:3000` edi, lekin ilova `4000`-portda ishlaydi (`Dockerfile`/`docker-compose.yml`/`.env`ga mos) — `api:4000`ga to'g'rilandi. Haqiqiy `docker compose` stack orqali HTTPS end-to-end tasdiqlandi (faqat `nginx -t` emas)
...
- [x] Loglash/monitoring — `nest-winston` ulandi: `src/config/winston.config.ts` (console + `logs/error.log` + `logs/combined.log`, NODE_ENV'ga qarab darajasi/formatlanishi o'zgaradi), `main.ts`da `NestFactory.create`ga `WinstonModule.createLogger(winstonConfig)` berildi. Bonus: oldindan yozilgan, lekin hech qayerda ulanmagan `LoggingInterceptor` endi `APP_INTERCEPTOR` sifatida global ro'yxatdan o'tkazildi (`app.module.ts`) — barcha HTTP so'rovlar endi winston orqali loglanadi
- [x] Umumiy unit-test qamrovi — `goals.service.spec.ts`, `channels.service.spec.ts`, `jwt.strategy.spec.ts`, `jwt-refresh.strategy.spec.ts`, `google.strategy.spec.ts` qo'shildi. Qolgan spec'siz joylar: controller'lar, gateway'lar (loyihada hech qaysi gateway uchun spec yo'q, repo konvensiyasiga mos)
...
- [x] **MUHIM (production bug, CI `test-e2e` orqali topildi):** `app.module.ts`da `TypeOrmModule.forRootAsync({ useFactory: databaseConfig })` — `inject: [ConfigService]` yo'q edi. Demak Nest `databaseConfig`ni **hech qanday argumentsiz** chaqirardi, `config` parametri `undefined` bo'lardi va `config.get('DB_HOST', ...)` xato berardi — **ilova haqiqiy Postgres bilan birorta ham marta muvaffaqiyatli boot bo'lmagan edi**. `MailerModule`/`BullModule`ning o'zidagi `forRootAsync`larda `inject: [ConfigService]` to'g'ri yozilgan edi — faqat `TypeOrmModule`da yo'q edi. `test/jest-e2e.json`ga `testTimeout` (doimiy, 30s) va `test:e2e` skriptiga `--forceExit` qo'shilgandan keyin bu xato aniq stack trace bilan tezda chiqdi (ilgari `disableLiburing` muammosi bilan birga osilib, ko'rinmasdi)
- [x] **`test-e2e` to'liq yashil bo'ldi.** Yo'lda yana bitta narsa topildi, lekin ATAYLAB kod o'zgartirilmadi: `GoogleStrategy` konstruktorida `passport-oauth2` `clientID`/`clientSecret` bo'sh bo'lsa **sinxron xato tashlaydi** — `.env.example`da bu ikkisi standart bo'yicha bo'sh, demak **haqiqiy Google OAuth credentials sozlanmagan har qanday muhitda butun ilova boot bo'la olmaydi** (faqat CI emas, local dev ham). Hozircha CI'ga soxta `GOOGLE_CLIENT_ID`/`SECRET` qo'shildi (xuddi `JWT_SECRET` kabi) — bu faqat CI'ni yashil qiladi, asosiy muammoni yo'q qilmaydi. **Qaror kerak:** OAuth sozlanmaganda `GoogleStrategy`/`/auth/google*` route'larini shartli ravishda o'chirib qo'yish kerakmi? Bu kod tuzilishini o'zgartiradigan mahsulot qarori, shuning uchun mustaqil qaror qabul qilinmadi
